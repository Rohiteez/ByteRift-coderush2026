// Aafno Pay: Core Shared Types & API Contract Definitions
...
export interface CreditScore {
  ...
  score: number; // 0 - 100
  tier: 1 | 2 | 3 | 4;
  maxLimitPaisa: number; // e.g. 400000 = NPR 4,000.00
...
export interface LoanRequest {
  id: string;
  borrowerId: string;
  borrowerPseudonym: string; // e.g. "Student #4892" for privacy
  universityName: string;
  faculty: string;
  purposeCategory: PurposeCategory;
  purposeDescription: string;
  amountRequestedPaisa: number; // e.g. 500000 = NPR 5,000.00
  amountFundedPaisa: number;
  tenureDays: number;
